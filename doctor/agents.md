...
```
Login (MSG91 SMS OTP)
  handlePhoneContinue() → window.sendOtp(91+phone) → step-otp
  handleVerifyOtp() → window.verifyOtp(otp) → access-token
    → POST /doctor/auth/verify-otp {access_token}
      ├─ existing doctor → token + localStorage → loadPatientsScreen()
      └─ new doctor (isNewDoctor) → step-register → handleCompleteProfile()
           → POST /doctor/complete-profile {name, specialization} → loadPatientsScreen()
...
| Method | Path                                            | Purpose                                                                        |
| ------ | ----------------------------------------------- | ------------------------------------------------------------------------------ |
| POST   | `/doctor/auth/verify-otp`                       | verify MSG91 access-token, login/auto-provision `{access_token}`               |
| POST   | `/doctor/complete-profile`                      | new doctor profile `{name, specialization}` (auth required)                    |
